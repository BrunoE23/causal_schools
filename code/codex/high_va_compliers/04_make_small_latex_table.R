...
x <- fread(input)
keep <- c('income_decile_imputed', 'father_educ_years_imputed',
          'mother_educ_years_imputed')
labels <- c(income_decile_imputed = 'Income decile',
...
stopifnot(nrow(x) == 9L, setequal(x$cutoff_percentile, c(25L, 50L, 75L)),
