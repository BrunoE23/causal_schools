###############################################################################
# Program-income VA vs institutional-quality VA correlation plot
###############################################################################
...
plot_path <- file.path(
  figure_dir,
  "program_income_x_institution_quality_va_correlation.png"
)
summary_csv_path <- file.path(
  table_dir,
  "program_income_institution_quality_va_correlation.csv"
)
...
institution_quality_values <- fread(
  school_values_path,
  select = c(
    "school_rbd",
    "analysis_sample",
...
institution_quality_values <- institution_quality_values[
  analysis_sample == "All" &
    outcome == "inst_certified_years_m1",
  .(
    school_rbd,
    institution_quality_va = controlled_value_added_centered_student,
    n_institution_quality = n_students_regression
  )
]
...
plot_dt <- merge(
  ...
  institution_quality_values,
  by = "school_rbd",
  all = FALSE,
  sort = FALSE
)
plot_dt <- plot_dt[complete.cases(program_income_va, institution_quality_va)]
plot_dt[, n_students := pmin(n_program_income, n_institution_quality, na.rm = TRUE)]
plot_dt[is.infinite(n_students), n_students := NA_real_]

if (nrow(plot_dt) < 3) {
  stop("Too few complete schools for program-income/institutional-quality correlation.", call. = FALSE)
}

fit <- stats::lm(institution_quality_va ~ program_income_va, data = plot_dt)
correlation <- stats::cor(plot_dt$program_income_va, plot_dt$institution_quality_va)
summary_dt <- data.table(
  comparison = "program_income_vs_institution_quality",
  x_axis = "Program income VA",
  y_axis = "Institutional quality VA",
...
annotation_dt <- data.table(
  ...
  y = stats::quantile(plot_dt$institution_quality_va, 0.96, na.rm = TRUE),
  label = summary_dt$label
)

correlation_plot <- ggplot(
  plot_dt,
  aes(x = program_income_va, y = institution_quality_va)
) +
  ...
  labs(
    ...
    y = "Institutional quality value added",
    title = "Program income VA vs institutional quality VA"
  ) +
