###############################################################################
# School VA correlation plots with program income always on the x-axis
###############################################################################
...
combined_plot_path <- file.path(
  figure_dir,
  "program_income_x_va_correlations.png"
)
summary_csv_path <- file.path(
  table_dir,
  "school_va_program_income_x_correlations.csv"
)
...
values_long <- rbindlist(
  list(program_income_values, values_long),
  use.names = TRUE
)
...
plot_specs <- unique(values_long[short_name != "program_income", .(
  short_name,
  plot_label,
  plot_order
)])
...
plot_dt <- rbindlist(lapply(seq_len(nrow(plot_specs)), function(i) {
  ...
  out <- values_wide[, .(
    school_rbd,
    program_income_value = program_income,
    y_value = get(y_name),
    n_students = pmin(n_program_income, get(n_col), na.rm = TRUE),
    comparison = y_name,
    comparison_label = y_label
  )]
  out[is.infinite(n_students), n_students := NA_real_]
  out[complete.cases(program_income_value, y_value)]
}), use.names = TRUE)

if (nrow(plot_dt) == 0) {
  stop("No complete program-income-x correlation data found.", call. = FALSE)
}

summary_dt <- plot_dt[, {
  fit <- stats::lm(y_value ~ program_income_value)
  .(
    correlation = stats::cor(program_income_value, y_value),
    n_schools = .N,
    slope = unname(stats::coef(fit)[["program_income_value"]]),
...
program_income_x_plot <- ggplot(plot_dt, aes(x = program_income_value, y = y_value)) +
  ...
  labs(
    ...
    title = "School value-added correlations with program income on the x-axis"
  ) +
...
for (i in seq_len(nrow(plot_specs))) {
  ...
  individual_plot <- ggplot(individual_dt, aes(x = program_income_value, y = y_value)) +
    geom_hline(yintercept = 0, color = "grey86", linewidth = 0.3) +
    geom_vline(xintercept = 0, color = "grey86", linewidth = 0.3) +
    ...
    labs(
      x = "Program income value added",
      y = comp_label,
      title = paste("Program income VA vs", comp_label)
    ) +
    theme_minimal(base_family = "serif", base_size = 11) +
    theme(
      plot.title = element_text(face = "bold"),
  ...
  ggsave(
    file.path(figure_dir, paste0("program_income_x_", comp, "_va_correlation.png")),
    individual_plot,
    width = 6.2,
    height = 5.1,
    dpi = 300
...
for (comp in plot_specs$short_name) {
  message("Wrote: ", file.path(figure_dir, paste0("program_income_x_", comp, "_va_correlation.png")))
}
