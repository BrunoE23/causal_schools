...
The script uses observed grade 5-8 enrollment rows from three years before through each student's first observed grade-8 cohort year. It uses the existing full-universe `tracking_univ8gr.RData` object from 2017 through the maximum `cohort_gr8` currently in `universe.csv`, and reads raw 2014-2016 enrollment files to complete the historical middle-school window for earlier grade-8 cohorts.
