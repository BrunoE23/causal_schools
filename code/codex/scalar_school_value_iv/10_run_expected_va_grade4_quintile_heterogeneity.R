...
# This script mirrors 09_run_expected_va_quintile_heterogeneity.R, but bins
# students by grade-4 math SIMCE quintile. It keeps the same SIMCE-8-linked
# 2019 sample so the comparison is not driven by students who lack grade-8
# SIMCE math scores.
###############################################################################
...
results_csv <- file.path(
  table_dir,
  "scalar_school_value_iv_expected_va_by_simce4_math_quintile_same_sample.csv"
)
main_csv <- file.path(
  table_dir,
  "scalar_school_value_iv_expected_va_by_simce4_math_quintile_same_sample_main.csv"
)
main_tex <- file.path(
  table_dir,
  "scalar_school_value_iv_expected_va_by_simce4_math_quintile_same_sample_main.tex"
)
...
simce[, `:=`(
  ...
  simce4_math_quintile = fifelse(
    is.na(simce4_math_decile),
    NA_integer_,
    as.integer(ceiling(simce4_math_decile / 2))
  )
)]
...
# Keep exactly the grade-8-linked sample used for the previous quintile table,
# then group those same students by their grade-4 math quintile.
analysis_dt <- dt[
  !is.na(simce8_math_quintile) &
    !is.na(simce4_math_quintile)
]
...
message("Running same-sample SIMCE 4 math-quintile expected-VA IV specs.")
...
quintile_rows <- main_wide[group != "All same sample"]
quintile_rows[, group_number := as.integer(sub("^Q", "", group))]
setorder(quintile_rows, group_number)

latex_rows <- unlist(lapply(seq_len(nrow(quintile_rows)), function(i) {
  row <- quintile_rows[i]
  c(
    paste(
      row$group,
