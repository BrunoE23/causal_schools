###############################################################################
# Grade-8 SIMCE math-quintile heterogeneity for expected-VA scalar IV
#
# CURRENT DEFAULT PATH:
# This script uses the expected-VA regression dataframe produced by
# 08_run_expected_va_scalar_iv.R. It does not use legacy prob_* / iszero_*
# controls.
###############################################################################
...
results_csv <- file.path(
  table_dir,
  "scalar_school_value_iv_expected_va_by_simce8_math_quintile.csv"
)
main_csv <- file.path(
  table_dir,
  "scalar_school_value_iv_expected_va_by_simce8_math_quintile_main.csv"
)
main_tex <- file.path(
  table_dir,
  "scalar_school_value_iv_expected_va_by_simce8_math_quintile_main.tex"
)
...
message("Reading SIMCE 8 math heterogeneity file: ", simce8_csv)
simce8 <- fread(
  simce8_csv,
  select = c(
    "mrun",
...
simce8[, mrun := as.numeric(mrun)]
dt[, mrun := as.numeric(mrun)]

dt <- merge(dt, simce8, by = "mrun", all.x = TRUE, sort = FALSE)
...
message("Running all-sample and SIMCE 8 math-quintile expected-VA IV specs.")
results <- rbindlist(lapply(seq_len(nrow(value_specs)), function(i) {
  rbindlist(c(
    list(run_scalar_iv(dt, value_specs[i], "All")),
    lapply(1:5, function(q) {
      run_scalar_iv(
        dt[simce8_math_quintile == q],
        value_specs[i],
        paste0("Q", q)
      )
    })
...
quintile_rows <- main_wide[group != "All"]
