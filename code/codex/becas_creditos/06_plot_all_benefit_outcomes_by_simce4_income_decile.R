...
))[order(outcome, sample, income_decile)]
...
summary_path <- file.path(table_dir, "all_benefit_outcomes_by_simce4_income_decile.csv")
figure_path <- file.path(figure_dir, "all_benefit_outcomes_by_simce4_income_decile.png")
...
plot <- ggplot(summary, aes(x = income_decile, y = share, colour = outcome, linetype = sample, shape = sample, group = interaction(outcome, sample))) +
  geom_line(linewidth = 0.8) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.10, linewidth = 0.35) +
  geom_point(size = 2.2) +
  ...
  labs(
    title = "Benefit applications and assignments by grade-4 SIMCE income decile",
    ...
    y = "Share of students",
    caption = "Observed at any point from 2020 to 2025; whiskers show 95% binomial confidence intervals."
  ) +
