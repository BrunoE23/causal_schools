...
outcomes <- fread(outcome_path, select = c("mrun_key", "grat_asignacion"))
...
plot_data <- plot_data[
  !is.na(income_decile) & income_decile %between% c(1, 10) & !is.na(grat_asignacion)
]
summarise_by_decile <- function(data, sample_label) {
  data[, .(
    students = .N,
    gratuidad_assignments = sum(grat_asignacion),
    share_gratuidad = mean(grat_asignacion)
...
summary[, se := sqrt(share_gratuidad * (1 - share_gratuidad) / students)]
summary[, `:=`(
  lower = pmax(0, share_gratuidad - 1.96 * se),
  upper = pmin(1, share_gratuidad + 1.96 * se)
)]

summary_path <- file.path(table_dir, "gratuidad_assignment_by_simce4_income_decile.csv")
figure_path <- file.path(figure_dir, "gratuidad_assignment_by_simce4_income_decile.png")
fwrite(summary, summary_path)

plot <- ggplot(summary, aes(x = income_decile, y = share_gratuidad, colour = sample, linetype = sample, shape = sample)) +
  ...
  labs(
    title = "Gratuidad assignment by grade-4 SIMCE income decile",
    subtitle = "Whole grade-8 universe and lottery sample\nAssignment observed at any point from 2020 to 2025",
    x = "Raw grade-4 SIMCE household-income decile",
    y = "Ever assigned gratuidad",
