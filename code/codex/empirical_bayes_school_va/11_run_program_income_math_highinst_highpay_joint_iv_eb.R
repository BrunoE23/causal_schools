###############################################################################
# Joint IV horse race for math, high-premium-institution, and high-premium-field
# EB VA.
#
# Outcome:
#   log(program_income_full)
#
# This script does not orthogonalize the school-value measures. It estimates a
# joint IV in which the three raw EB VA dimensions compete directly.
###############################################################################
...
school_output <- Sys.getenv(
  "JOINT_MATH_HIGHINST_HIGHPAY_SCHOOL_OUTPUT",
  unset = file.path(out_dir, "program_income_full_math_highinst_highpay_school_values.csv")
)
summary_output <- Sys.getenv(
  "JOINT_MATH_HIGHINST_HIGHPAY_SUMMARY_OUTPUT",
  unset = file.path(out_dir, "program_income_full_math_highinst_highpay_summary.csv")
)
correlation_output <- Sys.getenv(
  "JOINT_MATH_HIGHINST_HIGHPAY_CORRELATION_OUTPUT",
  unset = file.path(out_dir, "program_income_full_math_highinst_highpay_correlations.csv")
)
regression_output <- Sys.getenv(
  "JOINT_MATH_HIGHINST_HIGHPAY_REGRESSION_DF_OUTPUT",
  unset = file.path(out_dir, "program_income_full_math_highinst_highpay_joint_iv_regression_df.csv")
)
results_output <- Sys.getenv(
  "JOINT_MATH_HIGHINST_HIGHPAY_RESULTS_OUTPUT",
  unset = file.path(out_dir, "program_income_full_math_highinst_highpay_joint_iv.csv")
)
first_stage_output <- Sys.getenv(
  "JOINT_MATH_HIGHINST_HIGHPAY_FIRST_STAGE_OUTPUT",
  unset = file.path(out_dir, "program_income_full_math_highinst_highpay_joint_iv_first_stage.csv")
)
table_output <- Sys.getenv(
  "JOINT_MATH_HIGHINST_HIGHPAY_TABLE_OUTPUT",
  unset = file.path(out_dir, "program_income_full_math_highinst_highpay_joint_iv.tex")
)
...
component_values <- data.table(
  ...
  math_va_joint = weighted_center(work$math_va, w),
  highinst_va_joint = weighted_center(work$highinst_va, w),
  highpay_va_joint = weighted_center(work$highpay_va, w)
)
...
component_labels <- c(
  math_va_joint = "Math VA",
  highinst_va_joint = "High-premium-institution VA",
  highpay_va_joint = "High-premium-field VA"
)
...
summary_dt[, specification := "joint_raw_horse_race"]
...
component_school_values <- copy(component_values)
setnames(
  component_school_values,
  component_cols,
  c("school_value_math_va", "school_value_highinst_va", "school_value_highpay_va")
)

message("Reading DA probabilities and computing expected VA.")
...
prob_summary <- prob_long[, .(
  expected_math_va = sum(prob * fifelse(is.na(school_value_math_va), 0, school_value_math_va), na.rm = TRUE),
  expected_highinst_va = sum(prob * fifelse(is.na(school_value_highinst_va), 0, school_value_highinst_va), na.rm = TRUE),
  expected_highpay_va = sum(prob * fifelse(is.na(school_value_highpay_va), 0, school_value_highpay_va), na.rm = TRUE),
  mass_with_value_math_va = sum(prob * as.integer(!is.na(school_value_math_va)), na.rm = TRUE),
  mass_with_value_highinst_va = sum(prob * as.integer(!is.na(school_value_highinst_va)), na.rm = TRUE),
  mass_with_value_highpay_va = sum(prob * as.integer(!is.na(school_value_highpay_va)), na.rm = TRUE)
...
setnames(
  attended_values,
  c("school_value_math_va", "school_value_highinst_va", "school_value_highpay_va"),
  c("d_math_va", "d_highinst_va", "d_highpay_va")
)
estimation_df <- merge(
  estimation_df,
  attended_values,
...
setnames(
  offered_values,
  c("school_value_math_va", "school_value_highinst_va", "school_value_highpay_va"),
  c("offered_math_va", "offered_highinst_va", "offered_highpay_va")
)
estimation_df <- merge(
  estimation_df,
  offered_values,
...
estimation_df[, z_highinst_va := fifelse(
  is.na(rbd_treated_1R) | rbd_treated_1R == 0,
  0,
  offered_highinst_va
)]
estimation_df[, z_highpay_va := fifelse(
  is.na(rbd_treated_1R) | rbd_treated_1R == 0,
  0,
  offered_highpay_va
)]
estimation_df[, c("offered_math_va", "offered_highinst_va", "offered_highpay_va") := NULL]
...
endog <- c("d_math_va", "d_highinst_va", "d_highpay_va")
instruments <- c("z_math_va", "z_highinst_va", "z_highpay_va")
expected <- c("expected_math_va", "expected_highinst_va", "expected_highpay_va")
controls <- c(
  "factor(cohort_gr8)",
  "z_sim_mat_4to",
  "z_sim_leng_4to",
...
if (nrow(reg_dt) == 0) {
  stop("No complete observations for the joint IV.", call. = FALSE)
}
...
treatment_labels <- c(
  d_math_va = "Math VA",
  d_highinst_va = "High-premium-institution VA",
  d_highpay_va = "High-premium-field VA"
)
...
latex_table <- c(
  ...
  paste0(
    ...
    "The table reports a joint horse-race IV specification with three raw EB school-value ",
    "dimensions included together as endogenous regressors. Attended-school values are ",
    "instrumented with the corresponding first-round offered-school values, and the ",
    "regression controls for the DA-probability expected value of each dimension, cohort, ",
    "grade-4 SIMCE math and language, gender, and age. Heteroskedasticity-robust standard ",
