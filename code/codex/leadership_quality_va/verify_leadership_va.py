...
wide = pd.read_csv(DATA / "leadership_indices_and_measures.csv", float_precision="round_trip")
results = pd.read_csv(DATA / "leadership_va_correlations.csv")
params = pd.read_csv(DATA / "leadership_index_standardizations.csv")
dictionary = pd.read_csv(DATA / "leadership_analysis_metric_dictionary.csv")
...
assert set(wide.ROLE) == {"leadership"}
assert all(wide.groupby("ROLE").size() == 3682)

# Compact typed columns keep the 876,316 annual rows below a full raw-data load.
annual = pd.read_csv(DATA / "leadership_school_year_measures.csv.gz", usecols=[
    "RBD", "AGNO", "METRIC", "N_ROLE", "N_ELIGIBLE", "N_OBSERVED", "MEAN_OBSERVED", "VALUE"],
    dtype={"RBD": "int32", "AGNO": "int16", "METRIC": "category"})
annual["ROLE"] = "leadership"
...
period_check["expected"] = period_check["mean"].where(
    period_check.known.eq(7) & period_check.active.gt(0) & (period_check.valid / period_check.active).ge(.8))
period = pd.read_csv(DATA / "leadership_school_period_measures.csv").assign(ROLE="leadership").set_index(["RBD", "ROLE", "METRIC"])
...
index_checks = 0
for role in ["leadership", "leadership_history2015"]:
    is_alt = role.endswith("history2015")
    base_role = "leadership" if is_alt else role
    dt = wide[wide.ROLE.eq(base_role)].copy()
    ps = params[params.ROLE.eq(role)]
    components = ps.COMPONENT.tolist()
    if is_alt:
        dt["prior_role_years"] = dt.prior_role_years_2015
        dt["school_role_spell_years"] = dt.school_role_spell_years_2015
    ...
    score = "balanced_index_history2015" if is_alt else "balanced_index"
    np.testing.assert_allclose(balanced, dt.loc[selected, score], atol=1e-11)
    assert dt.loc[~selected, score].isna().all()
    if not is_alt:
        np.testing.assert_allclose(career, dt.loc[selected, "career_index"], atol=1e-11)
        np.testing.assert_allclose(credentials, dt.loc[selected, "credentials_index"], atol=1e-11)
        u, singular, vh = np.linalg.svd(z, full_matrices=False)
        pc1 = u[:, 0] * singular[0]
        direction = 1 if np.corrcoef(pc1, balanced)[0, 1] > 0 else -1
        pc1 = direction * (pc1 - pc1.mean()) / pc1.std(ddof=1)
        np.testing.assert_allclose(pc1, dt.loc[selected, "pca_index"], atol=1e-11)
        loadings = pd.read_csv(DATA / "leadership_pca_loadings.csv")
        np.testing.assert_allclose(direction * vh[0], loadings.loc[loadings.ROLE.eq(role), "PC1_LOADING"], atol=1e-11)
...
for (role, outcome), group in results.groupby(["ROLE", "OUTCOME"]):
    ...
    for row in group.itertuples():
        ...
        cor_checks += 1
        if row.METRIC not in ["balanced_index", "credentials_index", "career_index", "balanced_index_history2015", "mean_headcount"] or outcome not in [
...
for role, group in results.groupby("ROLE"):
    for pcol, qcol in [("PEARSON_P", "PEARSON_Q_BH"), ("ADJUSTED_P", "ADJUSTED_Q_BH")]:
        ...
        order = np.argsort(p)
        q = np.minimum.accumulate((p[order]*len(group)/np.arange(1, len(p)+1))[::-1])[::-1]
        check = np.empty(len(p)); check[order] = np.minimum(1., q)
        np.testing.assert_allclose(check, group.loc[eligible, qcol], atol=1e-12)

robust = pd.read_csv(DATA / "leadership_va_index_robustness.csv")
assert len(robust) == 144
...
for _, group in robust.groupby(["ROLE", "SUBSAMPLE"]):
    ...
    qvals = np.minimum.accumulate((pvals[order]*len(group)/np.arange(1, len(pvals)+1))[::-1])[::-1]
...
protected = pd.read_csv(DATA / "leadership_preserved_sources.csv")
for row in protected.itertuples():
    assert hashlib.md5(Path(row.PATH).read_bytes()).hexdigest() == row.MD5
summary = {"status": "passed", "prior_outputs_and_source_VA_unchanged": True, "school_index_scores_reconstructed": index_checks,
...
summary["report_input_sha256"] = {name: hashlib.sha256((DATA / name).read_bytes()).hexdigest() for name in [
    "leadership_indices_and_measures.csv", "leadership_va_correlations.csv", "leadership_va_index_robustness.csv",
    "leadership_index_diagnostics.csv", "leadership_pca_loadings.csv", "leadership_tenure_audit.csv",
    "leadership_va_outcome_dictionary.csv", "leadership_va_analysis_warnings.csv", "leadership_feature_coverage.csv",
    "leadership_history_verification.json"]}
print(json.dumps(summary, indent=2))
(DATA / "leadership_va_independent_verification.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
