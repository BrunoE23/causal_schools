...
input <- file.path(root, "data/clean/leadership_quality_va")
out <- file.path(root, "output/figures/leadership_quality_va")
dir.create(out, recursive = TRUE, showWarnings = FALSE)
results <- fread(file.path(input, "leadership_va_correlations.csv"))
...
common <- c("balanced_index", "career_index", "credentials_index", "pca_index", "prior_role_years",
            ...
            "prior_main_function_change_rate", "university_share")
...
for (role in "leadership") {
  extra <- c("teaching_title_share", "mean_headcount", "director_share", "log_va_students_per_staff")
  extra_labels <- c("Teaching qualification", "Mean leader headcount", "Director-code share", "Log VA students / leader")
...
wide <- fread(file.path(input, "leadership_indices_and_measures.csv"))
...
d[, ROLE := factor(ROLE, levels = "leadership", labels = "Leadership")]
...
p <- ggplot(d, aes(balanced_index, Y_Z)) + geom_hline(yintercept = 0, color = "#dce3e8") +
  ...
  facet_grid(ROLE ~ OUTCOME) + labs(x = "Balanced leadership-characteristics index (SD)", y = "School VA (SD within each panel)") +
...
ggsave(file.path(out, "balanced_index_scatter.png"), p, width = 10.2, height = 4.6, dpi = 220, bg = "white")
message("Created leadership correlation matrix and balanced-index scatter.")
