...
I link assignment records to administrative data on school enrollment, standardized achievement, and higher-education enrollment.
I estimate observational school value added for math, language, STEM enrollment, and institutional quality, measured by accreditation years, using pre-treatment controls that include grade-4 test scores, family background, and middle-school histories.
...
Heterogeneity estimates show that math value added is more predictive for higher-achieving baseline students, while higher-education value-added measures remain positive across baseline achievement.
Finally, an orthogonal value-added exercise shows that the component of STEM value added unrelated to math value added predicts STEM enrollment but not math achievement.
...
Families and policymakers might therefore care about school quality measures that capture more than achievement.
...
For higher-education outcomes, the pass-through is 0.689 for STEM enrollment and 1.153 for institutional quality, measured using institutional accreditation years.
...
The achievement estimates are close to comparable validation estimates in \textcite{angrist_leveraging_2017} based on lagged test-score models, even though the value-added measures here use a common pre-treatment control set that can also be applied to higher-education outcomes.
...
The higher-education value-added measures are more stable across the baseline achievement distribution.
...
\begin{minipage}[t]{0.49\textwidth}
  \begin{equation}
    \begin{aligned}
      Y_i^m ={}& \alpha^m + \theta^m A_i^m + \rho^m E_i^m \\
                &+ W_i'\delta^m + \eta_i^m,
    \end{aligned}
...
\begin{minipage}[t]{0.49\textwidth}
  \begin{equation}
    \begin{aligned}
      A_i^m ={}& a^m + \kappa^m O_i^m + \rho_A^m E_i^m \\
                &+ W_i'\delta_A^m + \nu_i^m.
    \end{aligned}
    \label{eq:first-stage}
  \end{equation}
\end{minipage}
Thus, $A_i^m$ is instrumented with $O_i^m$.
Here, $Y_i^m$ is student $i$'s realized value of outcome $m$.
$A_i^m$ is the value added of the school the student actually attends, $O_i^m$ is the value added of the school offered by the assignment mechanism, and $E_i^m$ is the expected value added implied by the student's assignment-probability vector.
$W_i$ is a parsimonious lottery-sample control vector containing cohort, gender, age, and grade-4 SIMCE math and language.
...
Table~\ref{tab:school-va-distribution} reports the student-weighted distribution of the four observational value-added measures.
The 90--10 percentile gap is 0.544 standard deviations for math achievement and 0.440 standard deviations for language achievement.
For the higher-education outcomes, the corresponding gap is 0.164 for STEM enrollment, or 16.4 percentage points, and 0.917 years for institutional quality.
The standard deviations are 0.258, 0.188, 0.078, and 0.375, respectively.
These distributions show that the school value-added measures contain economically meaningful variation and provide scale for interpreting the pass-through coefficients in Table~\ref{tab:main-four-va-results}.

\input{../../output/tables/results_section/school_va_distribution.tex}
...
In the bottom quintile, the math estimate is 0.145 and imprecise, while in the top quintile it is 1.317.
The language estimates follow a similar, though less monotone, pattern, increasing from 0.596 in the bottom quintile to 0.891 in the top quintile.
In contrast, the STEM enrollment estimates are positive but do not show the same baseline-achievement gradient: the bottom- and top-quintile estimates are 0.735 and 0.707.
The institutional-quality estimates are positive in all quintiles and are generally close to or above the unit-pass-through benchmark, with estimates of 1.075 in the bottom quintile and 1.201 in the top quintile.
This pattern suggests that the test-score value-added measures may be more predictive for higher-achieving baseline students, while the higher-education value-added measures remain informative across baseline achievement groups.
