...
Secondly, I validate these value added measurements for each school using the SAE lottery offers as exogenous variation.
...
They serve 90.6\% of the HS population. Pivate non-subsidized schools remain outside that mechanism and serve the remaining 9.4\%.
...
I do this by replicating the SAE assignment algorithm and repeatedly simulating counterfactual lottery draws, which determiens which students are at assignment risk.
As a diagnostic check, when I run the replicated algorithm using the original lottery numbers for the 2018--2020 regular grade-9 processes, I reproduce over 99\% of observed first-round assignments in each of the three cohorts. \footnote{Even though SAE runs two additional rounds, these complementary rounds are very limited in sample size. Since they assign vacancies in previously undersubscribbed schools, assignment risk is unlikely in these rounds.}
...
Figure \ref{fig:first-stage} makes this point by showing attendance rates in the top 100 schools most demanded, with different markers for applicants with and without first round offers.
...
For this exercise and the rest of the paper, I simplify high school attendance as the school where a student spends the most time after grade 8, with most recent spells breaking ties.
...
As a result, students who did not receive first-round offer might still end up attending a certain high-school via complementary rounds or an application cycle in a later year.
...
For instance, a student in cohort 2019, should apply that year to SAE if they want a change of school in the non-private system, is expected to start HS in 2020 and graduate in 2023.
...
From this universe I restrict students below 12 or above 16 in grade 8 and those who dont attend high school. 
...
As it can be seen in Panel A of Table \ref{tab:data-samples}, from the original 944,510 students in these cohorts, 
I keep the 757,999 for which I have a complete control vector with all the sources of information described above. 
Sample restrictions define on each row should be read cumulatively within each panel.
...
Out of that group only about 41\% (241,190) applied for a change of school through the SAE mechanism for a spot in grade 9\footnote{I consider only applications to grade 9 on the year the student reaches year 8 to avoid including on this sample students who changed due to failing a grade}.
In some cases, students (e.g. those applying to undersubscribbed schools) are guaranteed an offer to a certain school. 
...
For instance, they perform around 0.2 SD below in achievement tests in grade 4, have in average mothers with 1 fewer education year and perform 0.3 SD worse in exams to higher education.
The joint test clearly rejects these two populations to be similar.
...
with the respective joint test also rejecting these two groups to be similar. 
...
\input{../../output/tables/results_section/primary_five_va_distribution.tex}
...
The achievement estimates are close to one: a one-unit increase in the EB math value-added index raises math achievement by 1.197 standard deviations, while the analogous estimate for verbal is 1.031.
...
The estimated pass-through is 0.486 for admission-exam taking, 0.667 for high-premium-institution enrollment, 0.861 for high-premium-field enrollment, 1.060 for projected program income, and 0.584 for benefits or credit applications.
...
They also provide lottery-based evidence that high schools causally affect all seven primary outcomes studied here.
Lottery-induced movement toward schools with higher observational value added raises math achievement, verbal achievement, admission-exam taking, entry into high-premium institutions and high-premium fields, projected program income, and applications for benefits or credit, although at different rates depending on the outcome.
...
\subsection{Corrected Variance/Covariance Matrix}
...
Third, admission-exam taking and any financial-aid or benefits application move closely together (correlation 0.597), consistent with both capturing an earlier-stage engagement margin, but both are only weakly related to high-premium-institution and high-premium-field value added (0.075 and 0.060 for admission-exam taking; $-0.424$ and 0.050 for any application).
The negative correlation between any-application value added and high-premium-institution value added is the largest-magnitude relationship in the matrix outside the achievement block, and is discussed further below.
...
Most cells in the table are statistically distinguishable from zero at conventional levels, which given 42 directed pairs is itself informative, but three relationships are not: high-premium-field value added is not detectably related to either admission-exam-taking value added (0.074, SE 0.050, and 0.048, SE 0.032, in the two directions) or any-application value added (0.054, SE 0.036, and 0.046, SE 0.031), and log-income value added is not detectably related to any-application value added (0.015, SE 0.170, and 0.012, SE 0.135).
