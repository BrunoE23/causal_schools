...
%\input{../../output/tables/high_va_compliers/math_complier_ses_by_cutoff.tex}


%\input{../../output/tables/high_va_compliers/high_premium_field_complier_ses_by_cutoff.tex}
...
Table~\ref{tab:school-va-distribution} reports the scale of each student outcome and the dispersion of the corresponding school value-added measure for all seven primary outcomes. Binary outcomes and their value-added measures are expressed in percentage points, achievement outcomes in standard deviations, and projected income in log points. For school value added, the table compares the posterior EB standard deviation and 90--10 gap with the dispersion of the unshrunk estimates and the cross-cohort RSS estimate of latent dispersion.

\input{../../output/tables/results_section/primary_seven_va_distribution.tex}
...
The posterior EB standard deviation is 0.223 for math and 0.151 for verbal achievement; their 90--10 gaps are 0.496 and 0.375 standard deviations, respectively.
For the binary outcomes, the posterior EB standard deviations range from 5.6 percentage points for high-premium-field enrollment to 12.1 percentage points for high-premium-institution enrollment.
Projected-income value added has a posterior EB standard deviation of 0.055 log points and a 90--10 gap of 0.136 log points.
As expected, EB shrinkage produces less dispersion than either the unshrunk estimates or the RSS estimates of latent school heterogeneity.
...
\input{../../output/tables/empirical_bayes_school_va/va_2017_2020__sae_2018_2020/first_stage_kappa_table.tex}
...
\input{../../output/tables/empirical_bayes_school_va/va_2017_2020__sae_2018_2020/main_table.tex}
...
\input{../../output/tables/admin_type_va/admin_type_va.tex}

Table \ref{tab:admin-type-va} shows a regression of each of the seven dimensions of value added studied on administration type, high-school size, and curricular tracks.
...
(Maybe have a first regresion which just has the more crude controls, 
and hwo different private schools might be).

(Also separate big and small schools?)



Table~\ref{tab:staff-va-age-coverage} reports school-level regressions of each Empirical-Bayes value-added measure on these characteristics.
...
All regressions include the administration-type, school-size-category, and curricular-track indicators reported in Table~\ref{tab:admin-type-va}. These controls are suppressed from Table~\ref{tab:staff-va-age-coverage}; geography is not included.
...
%\input{../../output/tables/staff_va_compact_allschools/staff_va_compact.tex}
%\input{../../output/tables/staff_va_compact/staff_va_compact.tex}
%\input{../../output/tables/staff_va_all_roles_with_peers/staff_va_all_roles_with_peers.tex}
%\input{../../output/tables/staff_va_all_roles_no_peers/staff_va_all_roles_no_peers.tex}
\input{../../output/tables/staff_va_age_coverage/staff_va_age_coverage.tex}
...
%R^2 ?
The bottom rows of Table~\ref{tab:staff-va-age-coverage} summarize how much of each dimension these characteristics account for.
...
\input{../../output/tables/higher_ed_persistence/persistence_second_year_cross_va.tex}
...
\subsubsection{IV validation of RSS-implied gains}

I next assess whether the gains implied by the RSS covariance matrix predict causal effects in the lottery sample. For each directed pair of outcomes, I multiply the RSS projection of the destination value added on the origin value added by the destination outcome's same-outcome IV pass-through. This produces a predicted causal gain: the change in the destination outcome expected from a one-unit lottery-induced increase in the origin value added. I then compare this prediction with a direct cross-outcome IV estimate, obtained by instrumenting the origin value added of the attended school with the corresponding value added of the offered school while using the destination outcome as the dependent variable.

This exercise does not constitute a formal test that the RSS-implied and direct IV gains are equal. Such a test requires joint inference that accounts for estimation error and dependence across the RSS covariance estimates, the same-outcome pass-through estimates, and the cross-outcome IV estimates. The comparison instead provides evidence on whether the RSS-implied gains have empirical content outside the observational sample. To my knowledge, this is the first application to confront gains implied by a cross-outcome value-added covariance matrix with independent lottery-based causal estimates.

Table~\ref{tab:unshrunk-cross-iv-rss-validation} reports the complete set of estimates in levels. Panel A contains the direct cross-outcome IV estimates, while Panel B reports the corresponding RSS-implied causal gains. Reading the two panels together preserves the economically meaningful scale of each comparison and avoids the instability of ratios when the predicted gain is close to zero.

\input{../../output/tables/rss_debiased_varcov/unshrunk_cross_outcome_iv_rss_validation.tex}

Figure~\ref{fig:unshrunk-cross-iv-rss-validation} reports all 42 off-diagonal comparisons. The association is positive but imperfect: the correlation between the RSS-implied and direct IV gains is 0.562, and their signs agree in 69 percent of the directed pairs. These patterns provide partial validation of the implied-gain exercise. The RSS estimates contain information about which causal cross-outcome effects are larger and about their direction, but they do not reproduce every IV estimate closely.

\begin{figure}[!htbp]
\centering
\includegraphics[width=.72\textwidth]{../../output/figures/rss_debiased_varcov/unshrunk_cross_outcome_iv_rss_validation.pdf}
\caption{RSS-implied causal gains and direct cross-outcome IV estimates}
\label{fig:unshrunk-cross-iv-rss-validation}
\end{figure}

Table~\ref{tab:unshrunk-cross-iv-rss-notable-pairs} focuses on ten pairs chosen for their economic interpretation rather than their numerical agreement. A ratio of direct to implied gains equal to one denotes exact agreement. The academic pairs are reasonably close: the direct-to-implied ratios are 1.23 for math value added predicting verbal achievement and 0.85 in the reverse direction. The corresponding ratios are 1.21 when high-premium-institution value added predicts log income and 0.80 when high-premium-field value added predicts log income. The comparison is less successful for some access margins. In particular, the direct effect of exam-taking value added on financial-aid applications is only about one quarter of the RSS-implied gain. Thus, the validation is strongest for academic complementarities and the connection between postsecondary quality and projected earnings, while the mapping among earlier application decisions appears less stable.

\input{../../output/tables/rss_debiased_varcov/unshrunk_cross_outcome_iv_rss_notable_pairs.tex}

For completeness, Table~\ref{tab:unshrunk-cross-iv-rss-validation-ratios} reports the direct-to-implied ratio for every directed pair. Values near one indicate close agreement, values near zero indicate little direct IV effect relative to the RSS prediction, and negative values indicate opposite signs. The ratio matrix is best interpreted together with the levels in Table~\ref{tab:unshrunk-cross-iv-rss-validation}, since ratios can become very large when the RSS-implied gain is close to zero. Diagonal entries equal one mechanically and therefore provide no validation evidence.

\input{../../output/tables/rss_debiased_varcov/unshrunk_cross_outcome_iv_rss_validation_ratios.tex}
...
%\end{figure}
%\input{../../output/tables/results_section/simce4_math_quintile_primary_five_eb_contrasts_vs_q3.tex}
%\FloatBarrier
%
...
%\end{figure}
%\input{../../output/tables/results_section/primary_five_eb_gender_contrasts.tex}
%\FloatBarrier
%
%
%
...
%
%\input{../../output/tables/empirical_bayes_school_va/program_income_full_orthogonal_math_highinst_highpay_joint_iv.tex}
%
\FloatBarrier
...
\input{../../output/tables/compliance/application_rank_take_up_maximum_data.tex}
